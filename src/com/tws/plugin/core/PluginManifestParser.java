					...
					if ("manifest".equals(tag)) {
						...
						for (int index = 0; index < displays.length; index++) {
							...
							for (int i = 0; i < configs.length; i++) {
								...
								if (values.length != 2) {
									TwsLog.e(TAG, "ERROR plugin-display values=" + values);
									continue;
								}
								...
								} else if ("ctyle".equals(values[0])) {
									displayConfig.contentTyle = Integer.parseInt(values[1]);
								} else if ("content".equals(values[0])) {
									displayConfig.content = values[1];
								} else if ("icon".equals(values[0])) {
									displayConfig.iconResName = values[1];
									// 解析插件配置在宿主中需要显示的的图标
									if (!TextUtils.isEmpty(displayConfig.iconResName)) {
										final String[] resNames = displayConfig.iconResName
												.split(DisplayConfig.SEPARATOR_VALUE);
										for (String resName : resNames) {
											// 在assets/plugins/目录下面获取图标
											ZipEntry iconEntry = zipFile.getEntry(ManifestReader.ASSETS_PLUGINS
													+ resName + ManifestReader.ICON_SUFFIX);
											if (iconEntry != null) {
												PluginManagerHelper.addPluginIcon(resName,
														new BitmapDrawable(zipFile.getInputStream(iconEntry)));
												TwsLog.d(TAG, "addPluginIcon:" + resName);
											}
										}
									}
